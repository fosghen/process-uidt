...
class PeakFinder:
    ...
    def _process_row_analyze(freq: np.ndarray, data_row: np.ndarray, *args) -> float:
        try:
            ...
            params, _ = curve_fit(PeakFinder._laplace_func, freq, y_smooth, p0=[freq[index_max], 3], maxfev=1000)
            return params[0]
        except:
            return freq[0]

    @staticmethod
    def _process_row_reflectometer(freq: np.ndarray, data_row: np.ndarray, i: int, *args) -> float:
        print(args, i)
        try:
            params0, _ = curve_fit(PeakFinder._laplace_func, 
                                freq[:args[0][i]], 
                                data_row[:args[0][i]], 
                                p0=[freq[min(args[1][i])], 3],
                                maxfev=100,
                                bounds=([freq[0],          2],
                                        [freq[args[0][i]], 4]))
            ...
            params1, _ = curve_fit(PeakFinder._laplace_func,
                                freq[args[0][i]:],
                                data_row[args[0][i]:],
                                p0=[freq[max(args[1][i])], 3],
                                maxfev=100,
                                bounds=((freq[args[0][i]], 2),
                                        (freq[-1],         4)))
            return (params0[0] + params1[0]) / 2
        except:
            
            return (freq[args[1][i][0]] + freq[args[1][i][1]]) / 2

    @staticmethod
    def _find_peak_parallel(freq: np.ndarray, process_row: Callable, data: np.ndarray, *args) -> np.ndarray:
        return np.array(Parallel(n_jobs=-1)(delayed(process_row)(freq, data[i], i, args) for i in range(10)))
